...
`~/.cursor/config.toml` and silently skips a hook whose current hash drifts. After any edit,
commit, or merge touching `plugins/cursorclaw/hooks/*.json`, run `cxc doctor`. If it reports
