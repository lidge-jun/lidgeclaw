#!/usr/bin/env node
/**
 * cursorclaw CLI entry.
 *
 * Commands:
 *   cursorclaw enable              activate declared codex feature flags (config-guard)
 ...
 *   cursorclaw divergence          record divergence mode + candidate archive state
 *   cursorclaw loop                init/show/validate the project-local loop/goalplan substrate
 *   cursorclaw goalplan            (deprecated alias for loop)
 *   cursorclaw serve               run the bridge server (GUI static + API + messenger bots)
 *   cursorclaw service             install/uninstall/status the serve daemon (launchd)
 *   cursorclaw gui                 launch the cursorclaw web dashboard
 ...
 *
 * bin/cursorclaw.mjs is plain ESM with no build step, so it cannot import the TS
...
/** Delegate to the compiled crc-ops CLI (doctor/reset/hooks); returns its exit code. */
...
function runBgWake(args) {
  ...
  // up is components/. One was missing here and every root "crc bg" call — including
...
const TOP_LEVEL_HELP = [
  ...
  "  orchestrate <verb>             drive IPABCD state (try: crc orchestrate --help)",
  ...
  "  bg run|list|get|off            background tasks + completion wake (crc bg removal to uninstall)",
  "  serve                          run the bridge server",
  "  service                        install/uninstall/status the serve daemon",
  "  gui                            launch the web dashboard",
  "",
  "Agent notes:",
  "  Mutating PABCD commands require the current session id: crc orchestrate P --session <id>",
  "  Every --attest object names the edge it advances: {\"from\":\"<phase>\",\"to\":\"<phase>\",\"did\":\"...\"}",
  "  plus that edge's keys. Run crc orchestrate --help for the per-edge examples.",
  "  Use --json only on subcommands that document it, such as orchestrate status --json.",
  "  For command-specific help, start with: crc orchestrate --help or crc map --help.",
].join("\n");
...
function renderUnknownTopLevelCommand(cmd) {
  return `cursorclaw: unknown command '${cmd}'\nRun \`crc --help\` for usage.`;
}
...
function runRepoMap(args) {
  const scriptsDir = join(here, "..", "plugins", "cursorclaw", "skills", "repo-map", "scripts");
  ...
  if (!hasVenv && process.env.CODEXCLAW_MAP_BOOTSTRAP === "1" && !process.env.CODEXCLAW_PYTHON) {
    ...
    console.error(`cursorclaw map: bootstrapping venv at ${venvDir} (one-time)...`);
    ...
    if (mk.status === 0) {
      ...
      if (!hasVenv) {
        console.error("cursorclaw map: venv bootstrap failed; falling back.");
  ...
  // let "crc map" exit silently with no diagnostic at all.
  const notFound =
    (res.error && res.error.code === "ENOENT") || res.status === 9009 || res.status === 127;
  if (notFound) {
    console.error(
      `cursorclaw map: ${sel.cmd} could not be run (exit ${res.status ?? "spawn error"}).` +
...
if (isMain) switch (cmd) {
  ...
  case "-h":
    ...
    process.exit(0);
    break;
  // #47: `crc --version` was an unknown command, so the only way to learn which
  ...
  case "-v": {
    try {
      ...
      console.error(`crc --version: could not read the plugin manifest (${err.code ?? err.message})`);
      process.exit(1);
    }
    break;
  }
  ...
  case "freeze":
    ...
    // operator/main session when the interview is ready; cursorclaw never writes
    // the goal DB itself.
    process.exit(runPabcdState(process.argv.slice(2)));
    break;
  case "metric":
  ...
  case "service":
    ...
    break;
  case "gui": {
    const guiDir = join(here, "..", "plugins", "cursorclaw", "gui");
    ...
    if (!existsSync(guiVite) && !existsSync(rootVite)) {
      ...
      console.log(`cursorclaw gui: dependencies not installed. Run \`npm install\` in ${guiDir} first.`);
      process.exit(1);
    }
    console.log("cursorclaw gui: starting the dashboard (Vite will print the local URL)...");
    ...
    if (res.error) {
      ...
      console.error(`cursorclaw gui: ${hint}`);
      process.exit(1);
    }
