...
| `plugins/cursorclaw/bin/cursorclaw.mjs` | MODIFY — `config` 라우팅 |
| `config-guard/test/cli-config.test.ts` | NEW |
| `plugins/cursorclaw/test/packaging.test.mjs` | MODIFY — 신규 dist 산출물 존재 확인 |
...
출력 예:
    ...
    backup: /Users/…/.codex/config.toml.cursorclaw-2026-08-29T…bak

## 배포 경로

편집 대상은 **repo 소스**다(`/Users/jun/Developer/new/700_projects/codexclaw/plugins/cursorclaw/`).
플러그인 캐시(`~/.cursor/plugins/cache/codexclaw/codexclaw/<version>/`)를 직접 고치면 재설치로 사라진다.
반영 순서:

1. repo에서 소스 수정 → `npm run build`로 각 컴포넌트 `dist/` 갱신
2. `npm test` 전체
3. `node plugins/cursorclaw/scripts/inventory.mjs`로 인벤토리 갱신(신규 파일이 목록에 들어가야 한다)
...
| 5 | `config interview always` | `.cursorclaw/config.json` 기록 |
...
`bin/cursorclaw.mjs:149`의 config-guard 분기는 `[subcommand]` 하나만 넘긴다.
...
- `bin/cursorclaw.mjs`: `config`만 `process.argv.slice(2)` 전체를 넘기도록 특수화하고,
...
1. `activate`의 명명 규칙으로 config.toml 백업(`.cursorclaw-<timestamp>.bak`)
2. `setTableKey`로 키 쓰기
3. `tableKeys[managedKeyId] = { table, key, priorValue, appliedValue, setByCursorclaw }` upsert
   + `postActivateHash` 갱신
...
config-guard는 pabcd-state에 의존하지 않으며 다른 파일(`~/.cursor/config.toml`)을 소유한다.
...
앞선 절의 테스트 표 5행이 `.cursorclaw/config.json`을 적었다. 실제 구현 경로는 **저장소 루트 `codexclaw.json`**이다
...
| 파일 | 동작 |
|---|---|
| `bin/cursorclaw.mjs` | MODIFY — `config` 인자 보존 라우팅 + 하위 명령 분기 |
...
| `plugins/cursorclaw/test/cli-usage.test.mjs` | MODIFY — `config` 추가 |
