...
## MODIFY `plugins/cursorclaw/components/pabcd-state/src/comment-lint.ts` — NO

Wrong home: comment lint is deliberately fail-open and FSM-blind (R-9 note, line ~10).
Keep separation.

## ADD `plugins/cursorclaw/components/pabcd-state/src/idle-edit.ts`
 ...
 *  "[cursorclaw IDLE-EDIT] You are editing files with the PABCD FSM un-armed while a
...
## ADD hook registration `plugins/cursorclaw/hooks/pre-tool-use-advising-idle-edit.json`
...
- **Envelope (Low, PINNED)**: output MUST be
  ...
  additionalContext: "[cursorclaw IDLE-EDIT] ..." }` — allow+permissionDecisionReason
...
`git add -f plugins/cursorclaw/components/pabcd-state/dist/idle-edit.js`; at C:
...
- `plugins/cursorclaw/test/hook-e2e.test.mjs`: hooks length 14 → 15.
