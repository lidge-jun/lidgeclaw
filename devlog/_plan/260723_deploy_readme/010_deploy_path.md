...
|---|---|
| Repo-root `.agents/plugins/marketplace.json`, relative source | ✅ present, `codexclaw` / `./plugins/cursorclaw` |
...
- **B-CLI (High):** `cxc` does not ship in the marketplace payload — `bin/` and
  `cli/` sit at repo root, outside `plugins/cursorclaw/`. Marketplace users get
...
```
Cursor plugin install add <git-url>     # register marketplace
codex plugin add codexclaw@codexclaw       # install plugin
Cursor plugin install upgrade codexclaw # refresh git snapshot (update path)
codex plugin remove codexclaw@codexclaw    # uninstall plugin
Cursor plugin install remove codexclaw  # remove marketplace source
```

## Update UX to document (from LazyCodex mechanics)

- Update = `Cursor plugin install upgrade codexclaw` (git snapshot refresh).
...
- Doctor surface: `cxc` exposes doctor/reset via `cxc-ops`; CI gate =
  `node plugins/cursorclaw/scripts/gate.mjs`. Mention `cxc help` as the entry.
...
- **D2 — devlog public (Med).** 753 tracked files. Plugin payload is only
  `plugins/cursorclaw/`, so installs never receive devlog. RECOMMENDED: keep public
...
- **D6 — cxc CLI distribution (High, new at A).** Options: (a) ship a thin
  `plugins/cursorclaw/bin/cursorclaw.mjs` dispatcher in the payload and document
  `node "$PLUGIN_ROOT/bin/cursorclaw.mjs" ...` — works everywhere, no PATH, ugly;
...
- `Cursor plugin install upgrade` re-approval UX is Codex-side; if upstream
