...
- The live public distribution path is a **Git marketplace**: users run
  `Cursor plugin install add lidge-jun/codexclaw` (or the https URL), then
...
- [x] `.cursor-plugin/plugin.json` present with `name`, `version`,
...
- [ ] Optional polish (not blocking): `composerIcon` / `logo` / `logoDark` /
      screenshots under `plugins/cursorclaw/assets/` for directory presentation.
...
## Install smoke (fresh CURSOR_HOME)

Run under a temp home so the live `personal` dev registration is untouched:

```bash
CURSOR_HOME=$(mktemp -d) Cursor plugin install add https://github.com/lidge-jun/codexclaw.git
CURSOR_HOME=<same dir> codex plugin list --marketplace codexclaw --available
CURSOR_HOME=<same dir> codex plugin add codexclaw@codexclaw
```
...
The live `~/.cursor/config.toml` still registers this repo as marketplace
...
marketplace remove personal`, re-add via `Cursor plugin install add
...
`Cursor plugin install upgrade codexclaw` then reinstall/upgrade the plugin
as needed.
