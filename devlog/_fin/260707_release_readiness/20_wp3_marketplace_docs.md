...
`Cursor plugin install add` smoke against the public repo, checklist doc
...
| `plugins/cursorclaw/.cursor-plugin/plugin.json` | Add `repository`/`homepage`; `interface.defaultPrompt` string -> array (<=3 prompts, <=128 chars each); add `interface.websiteURL` (docs site). |
| `README.md` | Install line `codexclaw@personal -> codexclaw@codexclaw`. |
| `scripts/dev-symlink.sh` | `MARKETPLACE="personal" -> "codexclaw"` (line 17; cache path line 24 follows the var). Local-dev migration note goes in the checklist — the user's live `~/.cursor/config.toml` still registers `personal`; we do NOT touch it (outside goal write scope). |
...
3. `Cursor plugin install add https://github.com/lidge-jun/codexclaw.git`
   ...
   `CURSOR_HOME=$(mktemp -d)` so the live `personal` registration +
   `codexclaw@personal` install in `~/.cursor/config.toml` are untouched; the
