...
- 근거: `plugins/cursorclaw/skills/dev/SKILL.md:48`은 C0 기록 면제, C1은 기존 unit이 있을 때만 기록. `plugins/cursorclaw/skills/pabcd/SKILL.md:258`은 C0/C1 모두 기록하고 unit이 없으면 생성하도록 한다. `plugins/cursorclaw/skills/dev-scaffolding/references/implementation-log.md:75`도 후자를 따른다.
...
- 근거: `plugins/cursorclaw/skills/dev-testing/SKILL.md:195`의 QA ladder는 native 우선이며 200줄은 agbrowse의 built-UI driving을 금지한다. `/Users/jun/.codex/skills/agbrowse-browser/SKILL.md:164`의 QA routing은 같은 작업에 agbrowse 우선을 요구한다.
- 재현 입력: localhost UI에서 버튼 한 번 눌러 확인. 같은 작업에 agbrowse를 쓰라는 규칙과 쓰지 말라는 규칙이 함께 활성화된다.
- 별도 충돌: browser skill 44–48줄은 프로젝트에 `playwright-core` 설치를 안내하지만 `plugins/cursorclaw/skills/dev/SKILL.md:165`는 ad-hoc QA용 driver 설치를 금지한다. 이번 조사에서는 설치하지 않았다.
...
- 근거: `plugins/cursorclaw/skills/dev-diagram-viewer/upstream/visualize-upstream.md:10`은 1.0.11. `plugins/cursorclaw/skills/dev-diagram-viewer/reference/visualize-contract.md:54`는 2 MB, 74줄은 `::codex-inline-vis`를 사용한다.
...
- 실행 증거: `bash plugins/cursorclaw/skills/dev-diagram-viewer/upstream/sync-check.sh` → exit 1, `1.0.11 -> 1.0.29`, 양쪽 hash 불일치.
...
- 유형: 외부 계약 오류 / High / verified.
- 근거: `plugins/cursorclaw/skills/dev-security/references/asvs-checklist.md:1`은 ASVS 5.0이라 표기하지만 15줄은 V2 Authentication, 24줄은 V3 Session Management다. `plugins/cursorclaw/skills/dev-security/SKILL.md:123`도 V2/V3를 가리킨다.
...
- 유형: 내부 충돌 / Medium / verified.
- 근거: `plugins/cursorclaw/skills/dev-uiux-design/SKILL.md:402`은 density별 Iconoir/Hugeicons/Untitled UI/Lucide 등을 허용. `plugins/cursorclaw/skills/dev-frontend/SKILL.md:231`도 선택한 library 구현을 허용한다. 반면 `plugins/cursorclaw/skills/dev-frontend/references/stacks/react.md:199`는 Phosphor 또는 Radix import만 정확히 쓰라고 한다.
...
- 유형: 내부 충돌 / Medium / verified.
- 근거: `plugins/cursorclaw/skills/dev-uiux-design/SKILL.md:237`은 simple이면 세 dial 모두 감소. 266줄은 variance/motion만 감소하고 density는 유지하거나 증가.
...
- 유형: 내부 충돌 / Medium / verified.
- 근거: `plugins/cursorclaw/skills/dev-architecture/SKILL.md:79`와 `plugins/cursorclaw/skills/dev/SKILL.md:268`은 400줄 기준을 DEFAULT로 두고 사유가 있으면 초과 허용. `plugins/cursorclaw/skills/dev-code-reviewer/SKILL.md:167`은 500줄 초과를 이미 쪼개는 중이 아니면 blocking finding으로 정한다.
...
- 유형: 내부 충돌 / Medium / verified.
- 근거: `plugins/cursorclaw/skills/dev/SKILL.md:140`의 Database/schema/data → dev-data. `plugins/cursorclaw/skills/dev-data/SKILL.md:28`은 app CRUD/OLTP/transactional schema에는 활성화하지 말고 backend DB reference를 읽으라고 한다.
...
- 유형: 내부 충돌 / Medium / verified.
- 근거: `plugins/cursorclaw/skills/dev-testing/SKILL.md:371`은 C0/C1에 작은 검증만 요구. `plugins/cursorclaw/skills/dev-debugging/SKILL.md:192`와 240줄은 full suite를 요구하고, 196줄은 같은 유형의 모든 인스턴스를 고치라고 한다.
...
- 근거: `plugins/cursorclaw/skills/dev-scaffolding/SKILL.md:3`은 documentation scaffolding까지 활성화. 324–336줄은 post-scaffold에 dependency install/build/test/lint/두 번째 실행을 요구하며 실패하면 incomplete로 취급. `plugins/cursorclaw/skills/dev/SKILL.md:116`은 docs-only에 code gate가 없다고 명시한다.
...
- 근거: `plugins/cursorclaw/skills/dev-frontend/SKILL.md:39`, `plugins/cursorclaw/skills/dev-uiux-design/SKILL.md:78`의 `../../../../.cursorclaw/goalplans/.../090-synthesis.md`가 checkout에서 존재하지 않는다. `.gitignore:4`는 `.cursorclaw/`를 제외한다.
...
- 근거: `plugins/cursorclaw/skills/dev-architecture/SKILL.md:241`은 in-process domain entity constructor validation을 금지. `plugins/cursorclaw/skills/dev-testing/SKILL.md:307`은 domain invariants를 허용 guard로 열거한다.
...
- 유형: 범위 보완 / Medium / verified.
- 근거: `plugins/cursorclaw/skills/dev-backend/SKILL.md:310`은 기존 계약과 GraphQL/gRPC/SSE envelope 예외를 허용. `plugins/cursorclaw/skills/dev-data/SKILL.md:374`는 frontend용 data API에 standard envelope를 의무화한다. backend 271줄은 기존 오류 관례 우선인데 420줄은 AppError hierarchy를 checklist로 다시 고정한다.
...
- 유형: 범위 보완 / Medium / verified.
- 근거: `plugins/cursorclaw/skills/dev/SKILL.md:291`은 ESM을 의무화하면서 CommonJS require가 tree-shaking과 static analysis를 깨뜨린다고 일반화한다.
...
- 유형: 외부 계약 오류 / High / verified.
- 근거: `plugins/cursorclaw/skills/dev-data/SKILL.md:347`은 production query마다 EXPLAIN ANALYZE를 요구하지만 write query의 실행 부작용과 환경 제한을 적지 않는다.
...
- 유형: 내부 충돌 / High / verified.
- 근거: `plugins/cursorclaw/skills/dev-debugging/SKILL.md:37`은 어떤 fix 제안보다 RCA 완료를 먼저 요구. `plugins/cursorclaw/skills/dev-devops/SKILL.md:366`은 active incident에서 mitigation first, diagnosis second.
...
- 유형: 내부 충돌 / Medium / verified.
- 근거: `plugins/cursorclaw/skills/dev-diagram-viewer/SKILL.md:411`은 interaction verification optional. `plugins/cursorclaw/skills/pabcd/SKILL.md:184`는 stateful artifact의 첫 상호작용까지 실행하도록 한다. 현재 visualize 1.0.29 70줄도 primary interaction 갱신 확인을 요구한다.
...
- 근거: `plugins/cursorclaw/skills/dev-frontend/SKILL.md:335`은 Compiler를 React reference로 보낸다. 그러나 `plugins/cursorclaw/skills/dev-frontend/references/stacks/react.md:219`의 memo 안내에는 compiler 설정 여부의 분기가 없다. `plugins/cursorclaw/skills/dev-code-reviewer/SKILL.md:275`도 rerender 대응으로 memo를 먼저 나열한다.
...
- 근거: `plugins/cursorclaw/skills/dev-devops/SKILL.md:77`은 image HIGH/CRITICAL 무예외 차단. `plugins/cursorclaw/skills/dev-security/references/asvs-checklist.md:90`은 owner/기한/완화책을 둔 예외를 허용. `plugins/cursorclaw/skills/dev-security/references/supply-chain-sbom.md:125`도 기한 있는 예외를 언급한다.
...
- 유형: 범위 보완 / Medium / verified.
- 근거: `plugins/cursorclaw/skills/dev-scaffolding/SKILL.md:186`은 Python feature folder를 kebab-case로 만들고 `__init__.py`를 둔다. 200줄의 일반 폴더 규칙도 같다.
...
- 근거: `plugins/cursorclaw/skills/pabcd/SKILL.md:163`은 `agent_type: explorer`를 지정하도록 하지만 이번 호스트의 실제 spawn schema에는 agent_type이 없다. `structure/60_native_capabilities.md:88`의 update_plan도 이번 callable inventory에는 없다. `plugins/cursorclaw/skills/dev-diagram-viewer/SKILL.md:339`는 CDN script 포함 diagram을 항상 C1로 분류하지만 dev의 dependency/security 승격과 조건을 맞춰야 한다.
