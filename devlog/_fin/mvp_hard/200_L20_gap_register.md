...
| G11 | RESOLVED (WP6) | stale doc | `structure/INDEX.md:197` CLI Surface | FIXED 2026-06-30: added the `cxc freeze` row (delegates to pabcd-state CLI; writes `.cursorclaw/interview/freeze.json`). |
...
| G19 | RESOLVED (WP7+WP22) | test gap | hooks/*.json (5 of 6) | FIXED 2026-06-30: `test/hook-e2e.test.mjs` drives manifest hook entrypoints through their real dist command (`cli.js hook <event>`) with deterministic payloads + env (CURSOR_HOME=temp empty for the goal-DB read, controlled PATH for provider). WP7 added BEHAVIORAL coverage for 5 of 6 hooks — stop no-op release, goal-budget allow+deny, interview-in-goal allow, post-tool-use ledger side-effect, provider exit0+status line — plus a manifest→dist resolve check across all 6. Aristotle (gpt-5.4) A-gate supplied those contracts. WP22 closed the residual the Volta completion-audit caught: the 6th hook `user-prompt-submit` was only resolve-checked, never invoked. WP22 adds 2 behavioral cases (`plan this`→P directive envelope + `IPABCD: P` footer + orchestrationActive persist; no-trigger un-orchestrated → empty stdout, fail-closed, no state write); Ptolemy (gpt-5.4) A-gate confirmed the contract + no goals-DB dependency on the P path. Now all 6 manifest hooks are behaviorally exercised. |
...
| G22 | RESOLVED (WP7) | residue | `config-guard/src/cli.ts:18` (C6) | FIXED 2026-06-30: removed the dead prod export and relocated the guard into `config-guard/test/activate.test.ts` as a local helper (prod `main()` is meant to operate on the real `~/.cursor`, so the guard has no prod caller — Aristotle confirmed). |
