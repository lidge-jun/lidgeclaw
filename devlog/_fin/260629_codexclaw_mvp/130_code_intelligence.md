...
- `omo` plugin manifest는 `mcpServers`를 `./.mcp.json`로 위임하고, root `.mcp.json`에서 `lsp` 서버를 `node ./components/lsp-daemon/dist/cli.js mcp`로 연결한다. `codegraph`도 같은 root `.mcp.json`에 optional MCP로 등록되어 있다. 근거: `/Users/jun/Developer/new/700_projects/codexclaw/devlog/.lazycodex/plugins/omo/.cursor-plugin/plugin.json:45`, `/Users/jun/Developer/new/700_projects/codexclaw/devlog/.lazycodex/plugins/omo/.mcp.json:9`, `/Users/jun/Developer/new/700_projects/codexclaw/devlog/.lazycodex/plugins/omo/.mcp.json:25`
...
- codexclaw는 이미 plugin manifest에서 `mcpServers`를 `./.mcp.json`로 연결하고 있다. 근거: `/Users/jun/Developer/new/700_projects/codexclaw/plugins/cursorclaw/.cursor-plugin/plugin.json:25`
- 현재 codexclaw `.mcp.json`에는 `codexclaw` MCP 하나만 있고 `required: false`다. LSP/codegraph를 도입한다면 같은 파일의 `mcpServers`에 아래처럼 optional 서버를 추가하는 방식이 맞다. 근거: `/Users/jun/Developer/new/700_projects/codexclaw/plugins/cursorclaw/.mcp.json:1`
...
2. `ast-grep`은 별도 작업으로 `plugins/cursorclaw/skills/ast-grep` 포팅 후보를 만든다. binary 자동 설치는 opt-in으로 둔다.
