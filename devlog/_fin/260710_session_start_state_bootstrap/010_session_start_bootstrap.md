...
### MODIFY `plugins/cursorclaw/components/pabcd-state/src/state.ts`
...
- Create `.cursorclaw/sessions/` recursively; write the complete default JSON to a unique same-directory temp file; publish it with `linkSync(temp, final)` so final-path creation is atomic and no-clobber; remove the temp in `finally`.
...
### MODIFY `plugins/cursorclaw/components/pabcd-state/src/parse.ts`
...
### NEW `plugins/cursorclaw/hooks/session-start-bootstrapping-pabcd-state.json`
...
### MODIFY `plugins/cursorclaw/.cursor-plugin/plugin.json`
...
### MODIFY `plugins/cursorclaw/components/pabcd-state/test/state.test.ts`
...
### MODIFY `plugins/cursorclaw/components/pabcd-state/test/parse.test.ts`
...
### MODIFY `plugins/cursorclaw/test/hook-e2e.test.mjs`
...
- Force `ENOTDIR` by making `<cwd>/.cursorclaw` a regular file; prove exit 0, empty stdout, and no state file.
...
- `plugins/cursorclaw/components/pabcd-state/dist/state.js`
- `plugins/cursorclaw/components/pabcd-state/dist/hook.js`
- `plugins/cursorclaw/components/pabcd-state/dist/parse.js`
- `plugins/cursorclaw/components/pabcd-state/dist/cli.js`
...
5. Run `node plugins/cursorclaw/scripts/build.mjs`, freshness, packaging, `npm run gate`, and `npm test` in the isolated clone.
