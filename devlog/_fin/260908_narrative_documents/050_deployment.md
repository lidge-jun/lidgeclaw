...
|---|---|---|---|---|
| local (jun) | native plugin cache from marketplace clone ~/.cursor/.tmp/marketplaces/codexclaw | 0.2.23 | 24.17 / 0.153.4 | git pull marketplace to main SHA; native reinstall; doctor |
| macmini-cf | Git-mode cache (0.2.22), ~/.cursor/.tmp/marketplaces/codexclaw at c762542 | 0.2.22 | 22.22 / 0.146 | fetch main SHA; native reinstall; doctor; trust |
...
/Users/jun/Developer/new/700_projects/codexclaw-visual-release/.cursorclaw/evidence/
...
1. Reach probe: `ssh -o ConnectTimeout=8 -o BatchMode=yes <host> 'node -v; codex
   --version; ls ~/.cursor/plugins/cache/codexclaw/codexclaw'`.
2. Backup: copy current cache dir and private config preimage into the host-local
   backup dir (never off-host).
3. Update: `cd ~/.cursor/.tmp/marketplaces/codexclaw && git fetch origin && git
...
4. Verify: manifest version, required-file hash comparison with the payload, `cxc
   ...
   .cursorclaw/evidence/narrative-release-0.2.24/ssh/<host>-applied.json.
