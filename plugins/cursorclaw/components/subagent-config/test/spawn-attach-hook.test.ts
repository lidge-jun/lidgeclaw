...
test("mention normalization: plugin prefix is pinned to plugin.json name", () => {
  ...
  assert.equal(manifest.name, "cursorclaw");
  assert.equal(
    normalizeSkillMentions(`$${manifest.name}:dev`, SKILLS_DIR),
...
test("root recursion request mints a capability and keeps coordinator scope constraints", () => {
  ...
  assert.match(ui.message as string, /Do NOT run crc orchestrate, crc loop, or goal commands/);
...
test("concise dev metadata remains readable by the real leaf catalog", () => {
  ...
  assert.match(catalog, /^- dev: MUST USE for coding/m);
