...
export function loopArmDirective(platform: NodeJS.Platform = process.platform): string {
  const advance = platform === "win32"
    ? [
        ...
        "   `crc orchestrate <phase> --session <id> --attest-file .codexclaw/attest.json` —",
...
export function buildGoalIdleBlock(
  ...
  const startNext = platform === "win32"
    ? `Either start the next work-phase now: write the JSON with \`'{"from":"IDLE","to":"P","did":"<diff-level plan for the next work-phase>"}' | Set-Content -Encoding utf8 .codexclaw/attest.json\` then run \`crc orchestrate P --session ${sessionId} --attest-file .codexclaw/attest.json\``
  ...
  if (work) {
    ...
    lines.push(
      `The bound goalplan '${state.slug}' is EMPTY: register workPhases[]/criteria[] in .codexclaw/goalplans/${state.slug}/goalplan.json (schema in $crc-loop) so remaining work is durable and the E8 gate can pass.`,
    );
  } else if (!state.slug) {
    lines.push(
