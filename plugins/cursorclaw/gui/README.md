...
- **Subagents** — configure each role (explorer/reviewer/executor/architect): default model vs multi-model selection, and per-role prompt overrides. Backed by `.cursorclaw/subagents.json` via the `subagent-config` component.
