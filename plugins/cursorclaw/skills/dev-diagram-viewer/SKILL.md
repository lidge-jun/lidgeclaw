---
name: dev-diagram-viewer
description: "DEPRECATED — renamed to cxc-dev-visualizer. Use $cxc-dev-visualizer for visual documents, diagrams, HTML/SVG explainers and PDF reports."
metadata:
  deprecated: true
  redirect: cxc-dev-visualizer
---

# dev-diagram-viewer (DEPRECATED)

This skill was renamed to `$crc-dev-visualizer`. Every reference, asset and
verification rule (DIAGRAM-LAYOUT-01, DIAGRAM-RENDER-VERIFY-01, DIAGRAM-SYNTAX-01,
DIAGRAM-A11Y-01) now lives under `skills/dev-visualizer/`.

Use `$crc-dev-visualizer` instead. This folder ships only so older prompts,
devlogs and subagent attachments that name `dev-diagram-viewer` still
resolve to the current owner.
