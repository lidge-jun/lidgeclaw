#!/usr/bin/env node
/**
 * Payload-resident `crc` dispatcher (260724 fresh-install stabilization, WP1).
 *
 * WHY: the marketplace payload is only `plugins/cursorclaw/` — the repo-root
 * `bin/cursorclaw.mjs` (the PATH-level `crc` for repo checkouts) never ships.
 * This file gives every install a runnable command surface:
 *
 *   node "<payloadRoot>/bin/crc.mjs" orchestrate status --session <id>
 *
 * Injected directives resolve to exactly that invocation when `crc` is not on
 * PATH (components/crc-ops/src/crc-resolve.ts). Command set mirrors the root
 * dispatcher's payload-servable subset; a parity test pins the two tables
 * against each other (plugins/cursorclaw/test/payload-bin.test.mjs).
 *
...
const HELP = [
  "crc (payload dispatcher) — cursorclaw operator CLI",
  "",
  "Usage:",
  '  node "<payloadRoot>/bin/crc.mjs" <command> [args]',
  "",
  "PABCD / loop:",
  ...
  "  gui, map — use a git checkout of cursorclaw (README: Development)",
  "",
  "Agent notes:",
...
if (isMain) {
  ...
  // #47: `crc --version` was reported as an unknown command, so the only way to
  ...
  if (cmd === "gui" || cmd === "map") {
    console.log(
      `crc ${cmd}: available from a repo checkout only (github.com/lidge-jun/cursorclaw — see README Development section).`,
    );
    process.exit(1);
  }
  const component = COMMAND_TABLE[cmd];
  if (!component) {
    console.error(`crc: unknown command '${cmd}'\nRun with --help for usage.`);
    process.exit(1);
  }
