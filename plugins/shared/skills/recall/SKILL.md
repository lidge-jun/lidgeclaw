...
metadata:
  short-description: "Read-only recall search over ~/.cursor and ~/.codex: past chats (FTS-indexed) + memory store."
---

# recall — Past-Session Recall Search

Codex already persists every session (`~/.cursor/sessions/**/rollout-*.jsonl` and `~/.codex/sessions/**/rollout-*.jsonl`) and a
per-thread memory store (`~/.cursor/memories/` and `~/.codex/memories/`). This skill is the discipline for
...
`crc chat search` and `crc memory search` do not write Codex session files or the
memory store. They are not write-free against the sidecar: without `--no-refresh`,
`crc chat search` refreshes the sidecar index at `~/.codexclaw/recall/index.sqlite`,
and `crc chat index --rebuild` deletes and re-ingests that index. Pass
...
- `--home PATH` — search an alternate Codex home (default `$CURSOR_HOME` ?? `~/.cursor` or `$CODEX_HOME` ?? `~/.codex`).
- `--json` on `crc chat index` prints index status as JSON.
...
Chat (`crc chat search`, sidecar FTS index):
...
4. `crc chat search "<keywords>" --days 0 --no-tools` — find the conversation.
   Add `--context 2`. Add `--source all` if the work was delegated.
5. `crc memory search "<keywords>"` — durable summary. Omit `--no-chat` so empty
...
are not a substitute for `crc chat search --days 0`. `crc memory search` adds
...
Recall searches ONE Codex home per invocation — `$CURSOR_HOME ?? ~/.cursor` or `$CODEX_HOME ?? ~/.codex`,
...
`--no-refresh`. `crc chat index --status` shows freshness; `--rebuild` drops and
...
The rotation applies to the automatic injection ALONE. `crc chat search` and
`crc memory search` never consult it: the same query returns the same ranking
