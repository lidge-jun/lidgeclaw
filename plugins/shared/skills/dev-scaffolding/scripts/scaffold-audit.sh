...
# Checks whether a project follows the structural scaffold rules in this skill.
...
echo "🔍 Auditing '$DIR' against structural scaffold rules"
echo ""
...
if $IS_GO; then
  ...
  if [ -n "$FEATURE_DIRS" ]; then
    ...
    while IFS= read -r fdir; do
      [ ! -f "$fdir/index.js" ] && [ ! -f "$fdir/index.ts" ] && [ ! -f "$fdir/__init__.py" ] && MISSING_BOUNDARY=$((MISSING_BOUNDARY + 1))
    done <<< "$FEATURE_DIRS"
    if [ "$MISSING_BOUNDARY" -eq 0 ]; then
      check "Public boundary exports" "pass"
    else
      check "Public boundary exports ($MISSING_BOUNDARY feature(s) missing index.js/index.ts/__init__.py)" "fail"
    fi
  else
...
# 4. Source-of-truth docs (optional, convention-driven)
DOC_SIGNALS=0
for path in "$DIR/docs" "$DIR/plans" "$DIR/adr" "$DIR/architecture" "$DIR/README.md" "$DIR/AGENTS.md" "$DIR/CLAUDE.md"; do
  [ -e "$path" ] && DOC_SIGNALS=$((DOC_SIGNALS + 1))
done
if [ "$DOC_SIGNALS" -gt 0 ]; then
  check "Source-of-truth docs/signals present" "pass"
else
  check "Source-of-truth docs/signals absent (optional for small repos)" "warn"
fi
...
# 7. Agent/context instructions (optional but recommended)
if [ -f "$DIR/AGENTS.md" ] || [ -f "$DIR/CLAUDE.md" ]; then
  check "Agent/context instructions present" "pass"
else
  check "Agent/context instructions missing" "warn"
fi
