...
Examples:
  ...
  %(prog)s --chat-files main.py --other-files src/  # Specify chat vs other files
    ...
    parser.add_argument(
        ...
        type=int,
        default=4096,
        help="Maximum tokens for the generated map (default: 4096)"
    )
    ...
    try:
        ...
        reqs = Path(__file__).with_name("requirements.txt").resolve()
        print(
            f"cxc map: missing Python deps. Install: python3 -m pip install -r {reqs}",
            file=sys.stderr
        )
        sys.exit(3)
    ...
    try:
        ...
        if map_content:
            if args.verbose:
                print(
                    "map: "
                    f"{file_report.total_files_considered} files, "
                    f"{file_report.definition_matches} defs, "
                    f"{file_report.reference_matches} refs",
                    file=sys.stderr
                )
            
            print(map_content)
        else:
            print("cxc map: no map generated (no parseable source files)", file=sys.stderr)
            sys.exit(2)
